...
def import_train_test():
    train = pd.read_csv(f'{script_dir}/../Dataset/UNSW_NB15_training-set.csv')
    test = pd.read_csv(f'{script_dir}/../Dataset/UNSW_NB15_testing-set.csv')
    print("train: ", train.shape)
    print("test: ", test.shape)
    # Dropping the columns based on Feature Selection:
    # https://www.kaggle.com/khairulislam/unsw-nb15-feature-importance
    drop_cols = ['id',
                 'label']  #+ ['response_body_len', 'spkts', 'ct_flw_http_mthd', 'trans_depth', 'dwin', 'ct_ftp_cmd', 'is_ftp_login']
...
print("Column mismatch {0}, {1}".format(set(x_train.columns) - set(x_test.columns),
