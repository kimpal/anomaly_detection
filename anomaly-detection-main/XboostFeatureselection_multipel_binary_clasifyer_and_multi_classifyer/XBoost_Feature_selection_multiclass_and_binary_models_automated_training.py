# Code to run multiple models on different datasets and whit gridsearch and set parameters
import sys
import time
import pandas as pd
...
def select_features(x_train_inner, y_train_inner, x_test_inner, x_val_inner):
	...
	fs_inner = SelectFromModel(XGBClassifier(), threshold=0.007512621)
...
"""
# simpel evaluation whit out gir search could be used when best param in grid search is found
def evaluate_models(x_train, y_train, x_val, y_val, x_test, y_test, models):
...
# Create a RandomForestClassifier instance with the specified parameters
custom_rf_model = RandomForestClassifier(**params)

# Example Usage
models_to_evaluate = [
	("XGBoost", XGBClassifier(), {}),
   # ("Custom RandomForest", custom_rf_model, params),
	("DT", DecisionTreeClassifier(max_depth=2), {}),
	#("custom SVM",SVC(kernel='rbf',gamma='scale', degree=3, C=1.12), {}),
	#("KNN",KNeighborsClassifier(n_neighbors=3,), {}),
	#("LR",LogisticRegression(max_iter = 2000, random_state=10), {}),
	#("ANN",MLPClassifier(hidden_layer_sizes=(5,),solver='adam',learning_rate='adaptive', learning_rate_init=0.02,), {}),
	#("SVM", SVC(), {'C': 1.0, 'kernel': 'rbf'}),
]

# Assuming you have x_train, y_train, x_val, y_val, x_test, y_test defined
evaluate_models(x_train, y_train, x_val, y_val, x_test, y_test, models_to_evaluate)
"""
