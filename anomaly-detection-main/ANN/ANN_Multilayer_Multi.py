...
def ANN_predict(epoch_start, epoch_end, epoch_step, batch_size):
    ...
    for epoch in range(epoch_start, epoch_end + 1, epoch_step):
       ...
       # Label       
        ...
        ###--- ACCURACY & ERROR ---###
        train_accuracy, train_error = get_accuracy_error(y_train, used_train, "train")
        test_accuracy, test_error = get_accuracy_error(y_test, used_test, "test")
...
model = ANN_predict(epoch_start=20, epoch_end=20, epoch_step=10, batch_size=100)
...
df.to_csv('ANN_20E_2L(200-10)_AF(softsign-Sigmoid)_MULTI_label_10_classes.csv', index=False)
